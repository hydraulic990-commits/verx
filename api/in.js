export const _ORXJ = { runtime: "\x65\x64\x67\x65" };

const _KMZ = (process.env.T_D || "").replace(/\/$/, "");
...
export default async function fnTpwCoauT(req) {
  if (!TARGET_BASE) {
  ...
  try {
    ...
    const _tZjuNNqq = req.method;
    const _NzynmPeW = method !== "\x47\x45\x54" && method !== "\x48\x45\x41\x44";
